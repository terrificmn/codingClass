...
class SqlController (DbModel) :
    ...
    def validation(self, name, email):
        ...
        elif not re.search(name_regex, name):
            st.error('숫자 입력은 불가능 합니다.')
            return False
    ...
    def sql_select(self, chosenColumns, table, limit):
        ...
        query = " SELECT " + selectedColumn + " FROM " + table + " ORDER by title DESC LIMIT %s ; "
        #st.write(type(limit))
        data = (limit, )
    ...
    def sql_insert(self, name, email):
        # 연결
        conn = self.dbConnection()
        cursor = conn.cursor(dictionary=True)
