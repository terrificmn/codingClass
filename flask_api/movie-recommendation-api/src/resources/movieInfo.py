...
class MovieRecommend (Resource) :
    ...
    def post(self):
        ...
        # 문제가 없을 시(별점 있을 시 진행)
        user_id = 1004 # 테스트 끝날시 지우고 위에 주석 풀기
        ...
        connection = get_mysql_connection() # 테스트 끝나면 지우기
        # 판다스 df만들기
        user_ratings_df = pd.read_sql(query, connection)
        
        #print(user_ratings_df)

        # movie_correlations.csv 불러오기
        movie_correlations_df = pd.read_csv('movie_correlations.csv')
        
        #print(movie_correlations_df.head())


        #print(user_ratings_df['title'][0])
        ...
        for i in np.arange(0, len(user_ratings_df)):
            ...
            recommended_movies.columns = ['correlation']
        ...
        # # weight가 높은 것으로 정렬
        recommended_movies_list.reset_index(inplace=True)
