...
int main() {
	char fruit[20] = "strawberry"; //char 문자열 변수에 값을 넣으려면 변수[배열]을 적어줘야하고 인덱스는 문자열크기 +1 되어야한다 
    //자동으로 마지막에 \0인 null문자가 들어간다
	printf("딸기: %s\n", fruit);
	printf("딸기쨈: %s %s\n", fruit, "jam");
	
	//fruit = "banana"; //! 문자열 배열에는 직접 할당할 수가 없다. 처음에 선언할 때만 가능(c언어 특징) 
	//에러 발생함
	// 문자열 상수를 직접 입력은 안되지만 strcpy()함수를 이용하면 복사해서 만드는 것이 가능
...
//상수와 데이터 입력
    ...
    // scanf는 스페이스바 엔터로 구분을 지을 수 있다. 단, 위의코드처럼 %d%d%lf 식으로 사용 
