...
#3번-1 정규식 넣어주기
# [a-zA-Z_][a-zA-Z0-9_]+ 
#3번-2 정규식 넣어주기 \w 는 , 특수기호 괄호도 정의해주기 위해서 (앞에 \(이스케이프 시켜줌)  예: \(   \)
# \( (\w+) \)  ---이렇게 해주면 괄호포함 (1234) 숫자만 골라내는 정규식이 된다
#4번 마지막으로 합치기
# (?P<group1>[a-zA-Z_][a-zA-Z0-9_]+)\((\w)\))
#5번 적용
m = re.match('(?P<group1>[a-zA-Z_][a-zA-Z0-9_]+)\((?P<group2>\w+)\)', 'print(1234)') 
print(m.group('group1'))
print(m.group('group2'))
