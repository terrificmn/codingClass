...
테스트완료된 파일 내용은 아래, comment 와 함께 있는 버전 - 확인 OK
> 커멘트가 빠져 있는 버전은 아래쪽에 있음
```
...
커멘트를 뺀 버전  (순서는 조금 다르며, 기본값들은 빠져 있음) - 이것도 테스트 OK
```
anonymous_enable=NO
local_enable=YES
write_enable=YES
...
위의 환경 설정에 지정한 경로에다가 pem 파일 , key 파일을 만들어 주면 된다.  
